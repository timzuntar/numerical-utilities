...
def SingleRun(x,V_next,C_next,raster,measurement_times,measurement_concs,Nx,Ny,Nz,c0,cb,cmin,hlateral,hz,dt0,ROI_bounds,a):
    ke = x[0]
    Rprime = x[1]
    D = x[2]
    t = 0.0
...
def SingleRunParallelised(x,V_next,C_next,raster,measurement_times,measurement_concs,Nx,Ny,Nz,c0,cb,cmin,hlateral,hz,dt0,ROI_bounds,a,num_cpus):
    ke = x[0]
    Rprime = x[1]
    D = x[2]
    t = 0.0
    ...
    while(C_current.min() <= cmin):
        ...
        for cpu in range(num_cpus):
            ...
            timestep.append(IterateTimeStepParallelised.remote(V_current,D,Nzstart,Nzstop,Nx,Ny,Nz,dt,hlateral,hz,cb,c0))
        ...
        for cpu in range(num_cpus):
            ...
            pathological = ray.get(timestep[cpu])[1]
            if (pathological == True):
                return 1e6
            V_next[:,:,Nzstart:Nzstop] = ray.get(timestep[cpu])[0]  
...
#each cell has its own adsorbed concentration ranging from 0 to c0 (saturated).
#A cell's likelihood to adsorb additional molecules is modified by its own surface concentration (and possible that of neighboring cells)
#so in the event of a rupture the film regrowth is not entirely homogeneous.
#This is represented by a modified Kisliuk isotherm, also considering neighboring cells with a weighing parameter.
...
hlateral = 4.4e-6 #resolution of ellipsometry images is around 1 micron/pixel. 4x reduced resolution -> 4 microns/pixel
hz = 4.4e-6
c0 = 0.81e-6 #1 mg/square meter
...
#a = 0.25
a = 0.0
D = 8e-12   #rough approximation

initguess = [ke,Rprime,D]
parameterbounds = ((0.0,1e2),(0.0,1e2),(5e-12,3e-11))
...
godobject = (V_next,C_next,raster,measurement_times,measurement_concs,Nx,Ny,Nz,c0,cb,cmin,hlateral,hz,dt0,ROI_bounds,a,num_cpus)

optimized = optimize.minimize(SingleRunParallelised,initguess,bounds=parameterbounds,args=godobject,method="L-BFGS-B",options={"maxfun": 1, "maxiter": 1, "disp": True})
print(optimized.x)
...
signal = np.array(simulation_concs)
signal /= c0
outstring = "data/ROI1_optimized_%dpar_ke_%.3e_R_%.3e_a_%.3e_D_%.3e_dt0_%.2f.txt" % (len(initguess),ke,Rprime,a,D,dt0)
